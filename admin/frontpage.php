...
<div class="container-fluid" style="margin-top: 15px;">
    <ul class="nav nav-tabs" style="margin-bottom: 20px;">
        ...
        <li><a href="#content-tab" data-toggle="tab"><i class="fa fa-edit"></i> Edit Content</a></li>
    </ul>
...
function renderSections() {
    ...
    sections.forEach((section, index) => {
        ...
        container.append(`
            <div class="section-card" data-index="${index}" data-id="${section.id}">
                <div class="section-header" onclick="toggleSection(${index})">
                    ...
                    <div class="pull-right">
                        <label class="switch" style="margin: 0;" onclick="event.stopPropagation()">
                            <input type="checkbox" class="section-active" data-index="${index}" data-id="${section.id}" ${active ? 'checked' : ''}>
                            <span class="slider"></span>
                        </label>
                        <span class="status-label" style="margin-left: 10px; color: ${active ? '#5cb85c' : '#999'};">
    ...
    $('.section-active').on('change', function() {
        const checkbox = $(this);
        const card = checkbox.closest('.section-card');
        const sectionId = checkbox.data('id');
        const isActive = checkbox.is(':checked') ? 1 : 0;
        const statusLabel = card.find('.status-label');
        
        // Update label immediately
        statusLabel.text(isActive ? 'Active' : 'Inactive');
        statusLabel.css('color', isActive ? '#5cb85c' : '#999');
...
'<body>' +
    '<div class="container">' +
        ...
        '<\/div>' +
    '<\/div>' +
    '<scr' + 'ipt src="https://code.jquery.com/jquery-2.2.4.min.js"><\/scr' + 'ipt>' +
    '<scr' + 'ipt src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"><\/scr' + 'ipt>' +
'<\/body>' +
'<\/html>';
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
                ...
                loadSectDetails(); // Reload to show new image
            } else {
